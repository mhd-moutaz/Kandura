    ...
    <div style="background:white;border-radius:10px;padding:24px;box-shadow:0 2px 8px rgba(0,0,0,0.08);border:1px solid #e2e8f0;margin-bottom:24px;">
        <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
            ...
            <div style="text-align:center;padding:20px 30px;background:linear-gradient(135deg,#667eea,#764ba2);border-radius:12px;">
                <div style="font-size:14px;color:rgba(255,255,255,0.9);margin-bottom:4px;">Current Balance</div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
            ...
            <div style="background:#f0fdf4;padding:20px;border-radius:10px;border:2px solid #86efac;">
                ...
                <form action="{{ route('admin.wallet.deposit', $user) }}" method="POST">
                    ...
                    <div style="margin-bottom:12px;">
                        <label style="display:block;font-size:13px;color:#065f46;margin-bottom:6px;font-weight:500;">Amount (USD)</label>
                        <input type="number"
                               name="amount"
                               min="0.01"
                               max="100000"
                    ...
                    <div style="margin-bottom:16px;">
                        <label style="display:block;font-size:13px;color:#065f46;margin-bottom:6px;font-weight:500;">Description (Optional)</label>
            ...
            <div style="background:#fef2f2;padding:20px;border-radius:10px;border:2px solid #fca5a5;">
                ...
                <form action="{{ route('admin.wallet.withdraw', $user) }}" method="POST"
                      onsubmit="return confirm('Are you sure you want to deduct balance?');">
                    @csrf
                    <div style="margin-bottom:12px;">
                        <label style="display:block;font-size:13px;color:#991b1b;margin-bottom:6px;font-weight:500;">Amount (USD)</label>
                        <input type="number"
                               name="amount"
                               min="0.01"
                               max="{{ $wallet->balance }}"
                        ...
                        <div style="font-size:11px;color:#991b1b;margin-top:4px;">
                            Max: ${{ number_format($wallet->balance, 2) }}
                        </div>
                    </div>
                    <div style="margin-bottom:16px;">
                        <label style="display:block;font-size:13px;color:#991b1b;margin-bottom:6px;font-weight:500;">Description (Optional)</label>
                        <textarea name="description"
                                  rows="2"
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-times"></i> Deduct Balance
                    </button>
                </form>
            </div>
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="background:#f9fafb;border-bottom:2px solid #e5e7eb;">
                    ...
                    <th style="padding:12px;text-align:left;font-size:13px;color:#6b7280;font-weight:600;">Balance Before</th>
                    ...
                    <tr style="border-bottom:1px solid #f0f0f0;">
                        ...
                        <td style="padding:12px;">
                            ...
                            <span style="background:{{ $color['bg'] }};color:{{ $color['text'] }};padding:4px 10px;border-radius:6px;font-size:12px;font-weight:600;">
                                <i class="fas fa-{{ $color['icon'] }}"></i> {{ ucfirst($transaction->type) }}
                            </span>
                        </td>
                        ...
                        <td style="padding:12px;font-size:13px;color:#4a5568;">{{ $transaction->description ?? 'N/A' }}</td>
